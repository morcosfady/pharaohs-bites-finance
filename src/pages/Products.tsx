...
export function ProductsPage() {
  ...
  return (
    <div>
      ...
      <p className="mb-3 text-sm text-charcoal/60">Unit cost = ingredients (from recipes) + packaging + other direct cost{includeLabor ? " + owner labor" : ""}. Click a product to edit it and its recipe.</p>
...
export function ProductModal({ open, onClose, categories, product }: { open: boolean; onClose: () => void; categories: { id: string; name: string }[]; product?: Product }) {
  ...
  const submit = async () => {
    ...
    try {
      const row = (await write.mutateAsync(async () => unwrap(await supabase.from("products").upsert({ ...(product ? { id: product.id } : {}), ...f, slug, category_id: f.category_id || null }).select("id").single()))) as { id: string };
